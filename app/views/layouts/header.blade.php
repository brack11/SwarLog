<div class="navbar main-bar navbar-default">
    ...
    <div class="navbar-collapse collapse navbar-responsive-collapse">
        <ul class="nav navbar-nav">
            <li class=""><a href="{{ URL::route('indexDashboard') }}"><i class="glyphicon glyphicon-home"></i> <span>{{ trans('custom.logger') }}</span></a></li>
            @if (Sentry::check())
        ...
        <ul class="nav navbar-nav navbar-{{ (Config::get('syntara::config.direction') === 'rtl') ? 'left' : 'right' }}">
            ...
            <li><a href="{{ URL::route('showUser', $currentUser->id ) }}"><span class="text">{{ $currentUser->username }}</span></a></li>
